    ...
    >
      <van-cell v-for="(item, index) in list" :key="item.id">
        <template #title>
          ...
          类型： {{ item.type }}
          评分：{{ item.score }}
          <van-button type="primary" @click="cancelStar(item.id, index)">删除该记录</van-button>
...
export default {
  ...
  methods: {
    onLoad() {
      generatePost("/user/history", {user_id: this.$cookies.get("user_id")}).then((res) => {
    ...
    cancelStar(id: number, index: number) {
      generatePost("/user/delete_history", {user_id: this.$cookies.get("user_id"), dish_id: id}).then((res) => {
